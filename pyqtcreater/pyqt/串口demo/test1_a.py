...
class mycamera(QtWidgets.QMainWindow,Ui_camera.Q_MainWindow):
    ...
    def show_pic(self):
        ...
        if ret:
            ...
            ratio = max(width / self.show_window.width(), height / self.show_window.height())
...
class mywindow(QtWidgets.QMainWindow, UI.Ui_MainWindow):#继承两个类
    ...
    def change_stop_bit(self):
        ...
        if(buffer=='1'):
            self.serialNamePort.dataBits=self.serialNamePort.setStopBits(self.serialNamePort.StopBits.OneStop)
        elif(buffer=='1.5'):
            self.serialNamePort.dataBits=self.serialNamePort.setStopBits(self.serialNamePort.StopBits.OneAndHalfStop)
        elif(buffer=='2'):
            self.serialNamePort.dataBits=self.serialNamePort.setStopBits(self.serialNamePort.StopBits.TwoStop)
    ...
    def read_data(self):
        ...
        if(yes_or_no_show):
            try:
                self.show_window.appendPlainText(buffer.data().decode('utf-8'))
            except:
                self.show_window.appendPlainText(str(buffer.data()))
      
    @pyqtSlot()
    #发送
    def on_send_clicked(self):
        self.serialNamePort.write(self.send_window.text().encode('utf-8'))#把句学进行类型设置
        print('发送成功')
    ...
    def on_on_or_off_clicked(self):
        #初始化数据
        self.serialNamePort.portName=self.serialNamePort.setPortName(self.com.currentText())
        self.serialNamePort.baudRate=self.serialNamePort.setBaudRate(int(self.baud_rate_choose.currentText()))
        self.serialNamePort.dataBits=self.serialNamePort.setDataBits(self.serialNamePort.DataBits.Data8)
        self.serialNamePort.stopBits=self.serialNamePort.setStopBits(self.serialNamePort.StopBits.OneStop)
    ...
    def on_one_clicked(self):
        list1=[0x01,0x02]
        self.serialNamePort.write(list1)
