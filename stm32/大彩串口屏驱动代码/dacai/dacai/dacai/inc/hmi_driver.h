...
// 锁定设备配置，锁定之后需要解锁，才能修改波特率、触摸屏、蜂鸣器工作方式
...
// 记录控件 - 触发警告 void Record_ResetEvent(uint16_t screen_id, uint16_t control_id, uint16_t value, uint8_t *time);
// 记录控件 - 解除警告 void Record_Add(uint16_t screen_id, uint16_t control_id, uint8_t *record);
// 记录控件 - 添加常规记录 void Record_Clear(uint16_t screen_id, uint16_t control_id);
// 记录控件 - 清除记录数据 void Record_SetOffset(uint16_t screen_id, uint16_t control_id, uint16_t offset);
// 记录控件 - 设置记录显示偏移 void Record_GetCount(uint16_t screen_id, uint16_t control_id);
// 记录控件 - 获取当前记录数目
...
#if FIRMWARE_VER >= 921
/*!
 *  \brief     设置历史曲线采样数据值(单字节，uint8_t或int8_t)
 *  \param  screen_id 画面ID
 *  \param  control_id 控件ID
 *  \param  value 采样点数据
 *  \param  channel 通道数
